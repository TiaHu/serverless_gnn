...
class GCN(ScalableGNN):
    ...
    def forward(self, x: Tensor, adj_t: SparseTensor, batch_size, his_data, *args) -> Tuple[Any, List[Any]]:
        local_push_data = []
        if self.drop_input:
        ...
        for i, (conv, bn) in enumerate(zip(self.convs[:-1], self.bns)):
            ...
            x = torch.cat([x[:batch_size], his_data[i]], dim=0)

            # x[batch_size:][subset_mini_batch_pull_data] = his_data[i]
            # x = torch.cat([x[:batch_size], x[batch_size:]], dim=0)

            # x[:batch_size], n_id[:batch_size], offset, count
            # local_push_data.append(x[:batch_size].to(torch.device("cpu")))
        ...
        return self.lins[1](h), local_push_data
