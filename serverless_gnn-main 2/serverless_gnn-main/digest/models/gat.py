...
class GAT(ScalableGNN):
    ...
    def forward(self, x: Tensor, adj_t: SparseTensor, batch_size, his_data, *args) -> Tensor:
        local_push_data = []
        for i, (conv, history) in enumerate(zip(self.convs[:-1], self.histories)):
