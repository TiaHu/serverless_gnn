...
class Minibatch:
	...
	def init_all_params(self):
		...
		if self.rank == 0:
			...
			buffer_size = max([n_id.numel() for _, _, n_id, _, _ in self.eval_loader])
	...
	def train(self, his_data, if_old_his_data):
		...
		for i in range(self.local_epoch):
			...
			out, local_push_data = self.model(self.x, self.adj_t, self.batch_size, self.his_data, *self.args)
